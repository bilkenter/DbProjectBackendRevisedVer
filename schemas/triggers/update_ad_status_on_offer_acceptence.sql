...
CREATE OR REPLACE FUNCTION update_ad_status_on_offer_acceptance()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_update_ad_status_on_offer_acceptance
AFTER UPDATE ON PriceOffer
FOR EACH ROW
WHEN (OLD.offer_status IS DISTINCT FROM NEW.offer_status)
EXECUTE FUNCTION update_ad_status_on_offer_acceptance();
